...
// Web-safe map placeholder — MapLibreGL uses native modules that don't work on web.
// Renders an iframe map via MapTiler embed instead.
function WebMap({ center, storeCoord, customerCoord, riderCoord }) {
  const [lng, lat] = center

  const src = `https://api.maptiler.com/maps/streets-v2/?key=${MAPTILER_KEY}#14/${lat}/${lng}`
...
export default function TrackingScreen() {
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(pulseAnim, {
          toValue: 1.3,
          duration: 1000,
          useNativeDriver: false // useNativeDriver must be false on web
        }),
        Animated.timing(pulseAnim, {
          toValue: 1,
          duration: 1000,
  ...
  return (
    <View style={styles.container}>
      {/* Map — web-safe iframe version */}
      <View style={styles.mapContainer}>
        <WebMap
          center={mapCenter}
          storeCoord={order?.stores ? [order.stores.lng, order.stores.lat] : null}
          customerCoord={
